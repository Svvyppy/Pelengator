...
void initializeAdcCommon(ADC_HandleTypeDef* hadc)
{
...
void finalizeAdcInitialization(ADC_HandleTypeDef* hadc)
{
    checkHalStatus(HAL_ADC_Init(hadc));
    checkHalStatus(HAL_ADCEx_Calibration_Start(hadc, ADC_SINGLE_ENDED));
}

void initializeAdc1(ADC_HandleTypeDef* hadc)
{
    ADC_MultiModeTypeDef multimode = { 0 };
    ADC_ChannelConfTypeDef sConfig = { 0 };
    hadc->Instance                 = ADC1;
    initializeAdcCommon(hadc);
    finalizeAdcInitialization(hadc);
...
void initializeAdc2(ADC_HandleTypeDef* hadc)
{
    ADC_ChannelConfTypeDef sConfig = { 0 };
    hadc->Instance                 = ADC2;
    initializeAdcCommon(hadc);
...
void initializeAdc4(ADC_HandleTypeDef* hadc)
{
    ADC_ChannelConfTypeDef sConfig = { 0 };
    hadc->Instance                 = ADC4;
    initializeAdcCommon(hadc);
...
void initializeAdc5(ADC_HandleTypeDef* hadc)
{
    ADC_ChannelConfTypeDef sConfig = { 0 };
    hadc->Instance                 = ADC5;
    initializeAdcCommon(hadc);
...
void initializeDac4(DAC_HandleTypeDef* hdac)
{
...
void initializeOpamp4(OPAMP_HandleTypeDef* hopamp)
{
...
void initializeSamplingTimer(TIM_HandleTypeDef* htim)
{
...
void initializeServiceTimer(TIM_HandleTypeDef* htim)
{
    TIM_MasterConfigTypeDef sMasterConfig = { 0 };
...
void initializeUart1(UART_HandleTypeDef* huart)
{
